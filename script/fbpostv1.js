...
module.exports.config = {
    ...
    hasPrefix: true,
    aliases: ["fb1", "facebookpost1"],
    usage: "[fbpostv1 @mention | text | name] or [fbpostv1 text | name]",
    cooldown: 5,
};
...
module.exports.run = async function ({ api, event, args }) {
    try {
        ...
        const response = await axios.get(`https://hiroshi-rest-api.replit.app/canvas/fbpost?uid=${mentionId}&text=${encodeURIComponent(text)}&name=${encodeURIComponent(name)}`, { responseType: 'arraybuffer' });
